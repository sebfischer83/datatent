...
namespace Datatent.Core.Tree
{
	public sealed class TreeDiskNodeManager<K, V> : ITreeNodeManager<K, V>
	{
		...
		public TreeNode<K, V> RootNode => rootNode;
		...
		public TreeNode<K,V> CreateNewRoot (K key, V value, uint leftNodeId, uint rightNodeId)
		{
			...
			this.rootNode = node;
			recordStorage.Update (1u, LittleEndianByteOrderHelper.GetBytes(node.Id));
		...
		TreeNode<K, V> CreateFirstRoot ()
		{
			...
			recordStorage.Create (LittleEndianByteOrderHelper.GetBytes((uint)2));
