	...
	public sealed class TreeDiskNodeSerializer<K, V>
	{
		...
		byte[] VariableKeyLengthSerialize (TreeNode<K, V> node)
		{
			using (var m = new MemoryStream())
			{
				...
				m.Write (LittleEndianByteOrderHelper.GetBytes((uint)node.ParentId), 0, 4);
				// Followed by 4 bytes of how many entries
				m.Write (LittleEndianByteOrderHelper.GetBytes((uint)node.EntriesCount), 0, 4);
				// Followed by 4 bytes of how manu child references
				m.Write (LittleEndianByteOrderHelper.GetBytes((uint)node.ChildrenNodeCount), 0, 4);
				...
				for (var i = 0; i < node.EntriesCount; i++)
				{
					...
					m.Write (LittleEndianByteOrderHelper.GetBytes((int)key.Length), 0, 4);
				...
				for (var i = 0; i < node.ChildrenNodeCount; i++)
				{
					m.Write (LittleEndianByteOrderHelper.GetBytes((uint)childrenIds[i]), 0, 4);
				}
