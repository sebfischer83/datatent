...
using Datatent.Core.Streams;

namespace Datatent.Core.Extensions
{
	public static class StreamExtension
	{
		...
		public static float ExpectFloat (this Stream target)
		{
			...
			{
				return LittleEndianByteOrderHelper.GetSingle (buff);
			} else {
		...
		public static int ExpectInt32 (this Stream target)
		{
			...
			if (target.Read (buff) == 4)
			{
				return LittleEndianByteOrderHelper.GetInt32 (buff);
			} else {
				throw new EndOfStreamException ();
			}
		}
		...
		public static uint ExpectUInt32 (this Stream target)
		{
			...
			if (target.Read (buff) == 4)
			{
				return LittleEndianByteOrderHelper.GetUInt32 (buff);
			} else {
				throw new EndOfStreamException ();
			}
		}
		...
		public static long ExpectInt64 (this Stream target)
		{
			...
			{
				return LittleEndianByteOrderHelper.GetInt64 (buff);
			} else {
				throw new EndOfStreamException ();
			}
		}
		...
		public static double ExpectDouble (this Stream target)
		{
			...
			if (target.Read (buff) == 8)
			{
				return LittleEndianByteOrderHelper.GetDouble (buff);
			} else {
				throw new EndOfStreamException ();
			}
		}
